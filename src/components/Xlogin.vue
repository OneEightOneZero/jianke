<template>
	<div data-v-6764fe5f="" class="login-view view">
		<header data-v-d45bd762="" data-v-6764fe5f="" id="mallHeader">
			<div data-v-d45bd762="">
				<a data-v-d45bd762="" rel="nofollow" class="back-btn common-header-btn">
					<i data-v-d45bd762="" class="icon iconfont icon-back">
					</i>
				</a>
				<span data-v-d45bd762="">用户登录</span>
				<div data-v-d45bd762="" class="toggle-btn common-header-btn">
					<a data-v-d45bd762="" rel="nofollow" class="common-width">
						<i data-v-d45bd762="" class="icon iconfont icon-more">
							
						</i>
					</a>
				</div>
			</div>
			<nav data-v-d45bd762="" style="display: none;">
				<ul data-v-d45bd762="">
					<li data-v-d45bd762="" class="arrows-tag"></li>
					<li data-v-d45bd762="">
						<a data-v-d45bd762="" href="/" rel="nofollow">
							<i data-v-d45bd762="" class="icon iconfont icon-home"></i>
							<em data-v-d45bd762="">首页</em>
						</a>
					</li>
					<li data-v-d45bd762="">
						<a data-v-d45bd762="" href="/classify/classifyList" class="" rel="nofollow">
							<i data-v-d45bd762="" class="icon iconfont icon-types-search">
						</i>
						<em data-v-d45bd762="">分类搜索</em>
						</a>
					</li>
					<li data-v-d45bd762="">
						<a data-v-d45bd762="" href="/cart" class="" rel="nofollow">
							<i data-v-d45bd762="" class="icon iconfont icon-cart"></i>
							<em  data-v-d45bd762="">购物车</em>
						</a>
					</li>
					<li data-v-d45bd762="">
						<a data-v-d45bd762="" href="/user" class="" rel="nofollow">
							<i data-v-d45bd762="" class="icon iconfont icon-person-center"></i>
							<em data-v-d45bd762="">个人中心</em>
						</a>
					</li>
				</ul>
			</nav>
		</header>
		<div data-v-6764fe5f="" class="login-box">
			<section data-v-6764fe5f="" class="page">
				<div data-v-6764fe5f="" class="login-tab">
					<div data-v-6764fe5f="" class="mobile-login-btn active">手机号快捷登录</div>
					<div data-v-6764fe5f="" class="account-login-btn">账号密码登录</div>
				</div>
				<div data-v-6764fe5f="" class="login-wrap">
					<div data-v-6764fe5f="" class="mobile-login-box">
						<div data-v-6764fe5f="" class="tel-item"><input data-v-6764fe5f="" type="tel" placeholder="请输入您的手机号" maxlength="11"
							 class="inputStyle">
							<!---->
						</div>
						<div data-v-6764fe5f="" class="captchas-item" style="display: none;"><input data-v-6764fe5f="" type="text" id="ccc"
							 placeholder="图形验证码" class="inputStyle">
							<p data-v-6764fe5f="" class="img-btn"><img data-v-6764fe5f="" src=""></p>
							<!---->
						</div>
						<div data-v-6764fe5f="" class="code-item"><input data-v-6764fe5f="" type="text" placeholder="验证码" maxlength="6"
							 class="inputStyle"><button data-v-6764fe5f="" class="code-btn">获取验证码</button><button data-v-6764fe5f="" class="code-btn computing"
							 style="display: none;">重新发送(0秒)</button>
							<!---->
						</div>
					</div>
					<div data-v-6764fe5f="" class="account-login-box" style="display: none;">
						<div data-v-6764fe5f="" class="account-item"><input data-v-6764fe5f="" type="text" placeholder="请输入手机号/邮箱" class="inputStyle">
							<!---->
						</div>
						<div data-v-6764fe5f="" class="password-item"><input data-v-6764fe5f="" type="password" placeholder="请输入您的密码"
							 maxlength="16" class="inputStyle">
							<div data-v-6764fe5f="" class="pwd-switch"><img data-v-6764fe5f="" src="data:image/png;base64,"
								 alt=""></div>
							<!---->
						</div>
					</div>
					<div data-v-6764fe5f="" class="login-bar">
						<!----><button data-v-6764fe5f="" class="login-btn">登录</button></div>
					<div data-v-6764fe5f="" class="notice"><span data-v-6764fe5f="">温馨提示：未注册过健客的手机号，登录时会自动注册健客账号，且代表您已同意</span><a
						 data-v-6764fe5f="" href="/account/clause" class="">《健客服务条款》</a></div>
					<!---->
				</div>
				<div data-v-6764fe5f="" class="captchas_masking" style="display: none;"></div>
			</section>
			<!---->
			<div data-v-46c8ac9e="" data-v-6764fe5f="" class="loading-model hide"><img data-v-46c8ac9e="" src="data:image/gif;base64,"
				 alt="" class="hide"></div>
		</div>
	</div>
</template>

<script>
	export default {

	}
</script>

<style>

</style>
